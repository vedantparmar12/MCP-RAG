...
def call_llm(prompt: str, system_message: str = "You are a helpful assistant.", max_tokens: int = 150) -> str:
    """
    ...
    for retry in range(max_retries):
        try:
            if API_PROVIDER == "openai":
                ...
                # Gemini doesn't support batch embedding, so we'll do them individually
                embeddings = []
                for text in texts:
            ...
            if retry < max_retries - 1:
                ...
                for i, text in enumerate(texts):
                    try:
                        ...
                        # Add zero embedding as fallback - dimension depends on provider
                        embedding_dim = 1536 if API_PROVIDER == "openai" else 768
...
def create_embedding(text: str) -> List[float]:
    """
    Create an embedding for a single text using the configured API provider.
    ...
    try:
        ...
        if embeddings:
            ...
            # Return empty embedding if there's an error - dimension depends on provider
            embedding_dim = 1536 if API_PROVIDER == "openai" else 768
...
Please give a short succinct context to situate this chunk within the overall document for the purposes of improving search retrieval of the chunk. Answer only with the succinct context and nothing else."""
        ...
        context = call_llm(
            prompt=prompt,
            system_message="You are a helpful assistant that provides concise contextual information.",
            max_tokens=200
        )
...
"""
    ...
    try:
        return call_llm(
            prompt=prompt,
            system_message="You are a helpful assistant that provides concise code example summaries.",
            max_tokens=100
        )
    except Exception as e:
...
def extract_source_summary(source_id: str, content: str, max_length: int = 500) -> str:
    ...
    This function uses the configured LLM provider to generate a concise summary of the source content.
...
"""
    ...
    try:
        ...
        summary = call_llm(
            prompt=prompt,
            system_message="You are a helpful assistant that provides concise library/tool/framework summaries.",
            max_tokens=150
        )

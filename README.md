...
2. **Advanced RAG Strategies**: Implementing sophisticated retrieval techniques like contextual retrieval, late chunking, and others to move beyond basic "naive lookups" and significantly enhance the power and precision of the RAG system, especially as it integrates with Archon.
...
3. Copy and paste the entire contents of `crawled_pages.sql` into a new query
4. Run the query to create the necessary tables, functions, and indexes
5. Note your project URL and service key from Project Settings → API

### Step 3: Configuration

Create a `.env` file in the project root and configure it with your credentials (see Configuration section below for details).
...
TRANSPORT=sse
HOST=localhost  # Use 0.0.0.0 for Docker, localhost for direct Python execution
PORT=8051
...
USE_HYBRID_SEARCH=true     # Recommended: Combines vector + keyword search
USE_AGENTIC_RAG=false      # Enable for code example extraction
USE_RERANKING=true         # Recommended: Improves search result relevance
USE_KNOWLEDGE_GRAPH=false  # Enable for AI hallucination detection
...
# Neo4j Configuration (only needed if USE_KNOWLEDGE_GRAPH=true)
...
- The configuration above uses recommended settings for general documentation RAG
- For Windows users running directly (not Docker), use `HOST=localhost`
- The `crawled_pages.sql` file must be executed in your Supabase database before running the server
...
The server will start and listen on the configured host and port. You should see output like:
```
INFO:     Started server process [XXXX]
INFO:     Waiting for application startup.
INFO:     Application startup complete.
INFO:     Uvicorn running on http://localhost:8051 (Press CTRL+C to quit)
```
...
**1. "Access denied" or database connection errors:**
- Ensure your Supabase credentials are correct in the `.env` file
- Make sure you've run the `crawled_pages.sql` script in your Supabase database
...
- Check that port 8051 is not already in use
- Ensure all dependencies are installed: `uv pip install -e .`
- Verify your virtual environment is activated

**4. Crawling fails:**
- Some websites may block automated crawling
- Check that the website allows crawling in their robots.txt
- Try crawling a simple webpage first to test the setup

**5. Knowledge graph features not working:**
- Ensure Neo4j is running and accessible
- Check Neo4j connection details in the `.env` file
- Knowledge graph features require `USE_KNOWLEDGE_GRAPH=true`
...
3. Test with a simple webpage crawl once connected to an MCP client
...
3. Ensure your `.env` file contains all required values
4. Try running with minimal RAG strategies first (only `USE_HYBRID_SEARCH=true`)
